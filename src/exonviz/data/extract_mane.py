...
def attributes(line: str) -> Dict[str, str]:
    ...
    return {
        key: value for key, value in (record.split("=") for record in line.split(";"))
    }


def line_to_gff(line: str) -> Dict[str, Any]:
    spline = line.strip("\n").split("\t")
    d: Dict[str, Any] = {key: value for key, value in zip(gff_header, spline)}
    d["start"] = int(d["start"])
    d["end"] = int(d["end"])
...
def parse_gff(fname: str) -> Generator[Dict[str, Any], None, None]:
...
if __name__ == "__main__":
    ...
    for record in parse_gff(fname):
        if (
            ...
            and "MANE_Select" in record["attributes"]["tag"]
        ):
